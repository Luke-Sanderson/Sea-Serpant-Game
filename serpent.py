...
class Segment:
    ...
    def __init__(self, x, y,
                 canvas, id):
        self.drawing = canvas.create_oval(x, y, self.radius + x,
                                          self.radius + y, fill="blue")
        self.canvas = canvas
        self.id = id
    ...
    def move_serpent(self, segment_array):
        ...
        if current_coords[0] < 0:
            self.canvas.coords(self.drawing, 0, current_coords[1],
                               self.radius, current_coords[1] + self.radius)
            self.direction = 0
        elif current_coords[0] + self.radius > 1366:
            self.canvas.coords(self.drawing, 1366 - self.radius,
                               current_coords[1], 1366,
                               current_coords[1] + self.radius)
            self.direction = pi
        elif current_coords[1] + self.radius > 768:
            self.canvas.coords(self.drawing, current_coords[0],
                               768 - self.radius,
                               current_coords[0] + self.radius, 768)
            self.direction = 1.5 * pi

        if self.id != 0:
            prev = segment_array[self.id
                                 - 1].canvas.coords(segment_array[self.id
                                                                  - 1].drawing)
